...
class kostal_modbusquery:
    ...
    def ReadU8(self,myadr_dec):
        r1=self.client.read_holding_registers(myadr_dec,1,slave=71)
        U8register = BinaryPayloadDecoder.fromRegisters(r1.registers, byteorder=Endian.Big, wordorder=Endian.Big)
    ...
    def WriteR32(self,myadr_dec,value):
        ...
        myreadregister = BinaryPayloadDecoder.fromRegisters(myreadregister.registers, byteorder=Endian.Big, wordorder=Endian.Big)
        ...
        mybuilder = BinaryPayloadBuilder(byteorder=Endian.Big,wordorder=Endian.Big)
...
if __name__ == "__main__":
	...
	#
        ...
        Kostalquery = kostal_modbusquery(INVERTER_IP)
        ...
        print ("Battery charge percent                   :", Kostalquery.Qty['Battery actual SOC'][3]);
        print ("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")

        print("##+PV## ", int(LeftSidePowerGeneration))
        print("##+HOME## ",int(TotalHomeconsumption))
        print("##+GRID## ", int(PowertoGrid))
        print("##-BATTERYCHARGE## ", int(BatteryCharge))
        print("##+BATTERYCARGEPERCENT## ", int(Kostalquery.Qty['Battery actual SOC'][3]))
        ...
        if MQTT_Active == 1:
            try:
                ...
                broker_address=INVERTER_IP                                                 #IP-Adress of the mqtt broker we subscribe to
